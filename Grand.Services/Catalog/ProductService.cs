...
namespace Grand.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        /// </summary>
        /// <returns>Products</returns>
        public virtual IList<Product> GetAllProductsDisplayedOnHomePageTrending()
        {
            var builder = Builders<Product>.Filter;
            var filter = builder.Eq(x => x.Published, true);
            filter = filter & builder.Eq(x => x.ShowOnHomePageTrending, true);
            filter = filter & builder.Eq(x => x.VisibleIndividually, true);
            var query = _productRepository.Collection.Find(filter).SortBy(x => x.DisplayOrder).ThenBy(x => x.Name);
            var products = query.ToList();
            return products;
        ...
        /// <summary>
        /// Gets products by coupon
        /// </summary>
        /// <param name="couponId">Product identifiers</param>
        /// <returns>Products</returns>
        public virtual IPagedList<Product> GetProductsByCoupon(string couponId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = from c in _productRepository.Table
                        where c.AppliedCoupons.Any(x => x == couponId)
                        select c;
        ...
        public virtual void DeleteCoupon(string couponId, string productId)
        {
            if (string.IsNullOrEmpty(couponId))
                throw new ArgumentNullException("coupon");

            var updatebuilder = Builders<Product>.Update;
            var update = updatebuilder.Pull(p => p.AppliedCoupons, couponId);
            _productRepository.Collection.UpdateOneAsync(new BsonDocument("_id", productId), update);
        ...
        public virtual void InsertCoupon(string couponId, string productId)
        {
            if (String.IsNullOrEmpty(couponId))
                throw new ArgumentNullException("coupon");

            var updatebuilder = Builders<Product>.Update;
            var update = updatebuilder.AddToSet(p => p.AppliedCoupons, couponId);
            _productRepository.Collection.UpdateOneAsync(new BsonDocument("_id", productId), update);
        ...
        //sorting product list for admin
        public virtual IPagedList<Product> GetProductsByFlashSale(string flashSaleId, int pageIndex = 0, int pageSize = int.MaxValue)
        {
            var query = from c in _productRepository.Table
                        where c.AppliedFlashSales.Any(x => x == flashSaleId)
                        select c;
        ...
        // delete flashSale product for admin
        public virtual void DeleteFlashSaleProduct(string flashSaleId, string productId)
        {
            if (string.IsNullOrEmpty(flashSaleId))
                throw new ArgumentNullException("flashSale");

            var updatebuilder = Builders<Product>.Update;
            var update = updatebuilder.Pull(p => p.AppliedFlashSales, flashSaleId);
            _productRepository.Collection.UpdateOneAsync(new BsonDocument("_id", productId), update);
        ...
        // add/insert flashSale product for admin
        public virtual void InsertFlashSaleProduct(string flashSaleId, string productId)
        {
            if (String.IsNullOrEmpty(flashSaleId))
                throw new ArgumentNullException("flashSale");

            var updatebuilder = Builders<Product>.Update;
            var update = updatebuilder.AddToSet(p => p.AppliedFlashSales, flashSaleId);
            _productRepository.Collection.UpdateOneAsync(new BsonDocument("_id", productId), update);
        ...
        public virtual Product GetLDGiftInfo(string id)
        {
            var query = from c in _productRepository.Table
                        where c.Id == id && c.ProductTypeId == 30
                        select c;
