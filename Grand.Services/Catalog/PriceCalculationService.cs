...
namespace Grand.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        [Serializable]
        protected class CouponProductPriceForCaching
        {
            public CouponProductPriceForCaching()
            {
                AppliedCouponIds = new List<string>();
            }
            public decimal Price { get; set; }
            public decimal AppliedCouponAmount { get; set; }
            public List<string> AppliedCouponIds { get; set; }
        }
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToProduct(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;

            if (product.AppliedCoupons.Any())
            {
                foreach (var appliedCoupon in product.AppliedCoupons)
                {
                    var coupon = _couponService.GetCouponById(appliedCoupon);
                    if (coupon != null)
                    {
                        var validCoupon = _couponService.ValidateCoupon(coupon, customer);
                        if (validCoupon.IsValid &&
                            coupon.CouponType == CouponType.AssignedToSkus)
                            allowedCoupons.Add(new AppliedCoupon()
                            {
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToAllProduct(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;

            var coupons = _couponService.GetAllCoupons(CouponType.AssignedToAllProducts);
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToCategories(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;
            ...
            {
                var category = _categoryService.GetCategoryById(productCategory.CategoryId);
                if (category != null && category.AppliedCoupons.Any())
                {
                    foreach (var appliedCoupon in category.AppliedCoupons)
                    {
                        var coupon = _couponService.GetCouponById(appliedCoupon);
                        if (coupon != null)
                        {
                            var validCoupon = _couponService.ValidateCoupon(coupon, customer);
                            if (validCoupon.IsValid && coupon.CouponType == CouponType.AssignedToCategories)
                                allowedCoupons.Add(new AppliedCoupon()
                                {
                                    CouponCode = validCoupon.CouponCode,
                                    CouponId = coupon.Id,
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToManufacturers(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;
            ...
            {
                var manufacturer = _manufacturerService.GetManufacturerById(productManufacturer.ManufacturerId);
                if (manufacturer != null && manufacturer.AppliedCoupons.Any())
                {
                    foreach (var appliedCoupon in manufacturer.AppliedCoupons)
                    {
                        var coupon = _couponService.GetCouponById(appliedCoupon);
                        if (coupon != null)
                        {
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToVendors(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;
            ...
            if (!string.IsNullOrEmpty(product.VendorId))
            {
                ...
                if (vendor != null)
                {
                    if (vendor.AppliedCoupons.Any())
                    {
                        foreach (var appliedCoupon in vendor.AppliedCoupons)
                        {
                            var coupon = _couponService.GetCouponById(appliedCoupon);
                            if (coupon != null)
                            {
                                var validCoupon = _couponService.ValidateCoupon(coupon, customer);
                                if (validCoupon.IsValid &&
                                         coupon.CouponType == CouponType.AssignedToVendors)
                                    allowedCoupons.Add(new AppliedCoupon()
                                    {
                                        CouponCode = validCoupon.CouponCode,
                                        CouponId = coupon.Id,
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCouponsAppliedToStores(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;
            ...
            if (product.LimitedToStores == false)
            {
                //Products that don't have any Store (inside Stores collection) shouldn't have any Coupons by Store
            }
            else
            {
                //if it is limited to store, it means it should have at least one Store assigned
                foreach (var storeID in product.Stores)
                {
                    ...
                    {
                        var store = _storeService.GetStoreById(storeID);
                        if (store != null && store.AppliedCoupons.Any())
                        {
                            foreach (var appliedCoupon in store.AppliedCoupons)
                            {
                                var coupon = _couponService.GetCouponById(appliedCoupon);
                                if (coupon != null)
                                {
                                    var validCoupon = _couponService.ValidateCoupon(coupon, customer);
                                    if (validCoupon.IsValid &&
                                             coupon.CouponType == CouponType.AssignedToStores)
                                        allowedCoupons.Add(new AppliedCoupon()
                                        {
                                            CouponCode = validCoupon.CouponCode,
                                            CouponId = coupon.Id,
        ...
        protected virtual IList<AppliedCoupon> GetAllowedCoupons(Product product, Customer customer)
        {
            var allowedCoupons = new List<AppliedCoupon>();
            if (_catalogSettings.IgnoreCoupons)
                return allowedCoupons;

            //coupons applied to products
            foreach (var coupon in GetAllowedCouponsAppliedToProduct(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);

            //coupons applied to all products
            foreach (var coupon in GetAllowedCouponsAppliedToAllProduct(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);

            //coupons applied to categories
            foreach (var coupon in GetAllowedCouponsAppliedToCategories(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);

            //coupons applied to manufacturers
            foreach (var coupon in GetAllowedCouponsAppliedToManufacturers(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);

            //coupons applied to vendors
            foreach (var coupon in GetAllowedCouponsAppliedToVendors(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);

            //coupons applied to stores
            foreach (var coupon in GetAllowedCouponsAppliedToStores(product, customer))
                if (!allowedCoupons.Where(x => x.CouponId == coupon.CouponId).Any())
                    allowedCoupons.Add(coupon);
        ...
        protected virtual decimal GetCouponAmount(Product product, Customer customer, decimal productPriceWithoutCoupon, out List<AppliedCoupon> appliedCoupons)
        {
            if (product == null)
                throw new ArgumentNullException("product");

            appliedCoupons = null;
            decimal appliedCouponAmount = decimal.Zero;

            //we don't apply coupons to products with price entered by a customer
            if (product.CustomerEntersPrice)
            ...
            var allowedCoupons = GetAllowedCoupons(product, customer);

            //no coupons
            if (!allowedCoupons.Any())
                return appliedCouponAmount;

            appliedCoupons = _couponService.GetPreferredCoupon(allowedCoupons, customer, product, productPriceWithoutCoupon, out appliedCouponAmount);
            return appliedCouponAmount;
        }
        ...
        #region coupons Methods 

       
        public virtual decimal GetCouponFinalPrice(Product product,
            Customer customer,
            decimal additionalCharge = decimal.Zero,
            ...
            int quantity = 1)
        {
            ...
            return GetCouponFinalPrice(product, customer, additionalCharge, includeCoupons,
                quantity, out couponAmount, out appliedCoupons);
        }
        ...
        public virtual decimal GetCouponFinalPrice(Product product,
            ...
            bool includeCoupons,
            int quantity,
            out decimal couponAmount,
            out List<AppliedCoupon> appliedCoupons)
        {
            return GetCouponFinalPrice(product, customer,
                additionalCharge, includeCoupons, quantity,
                null, null,
                out couponAmount, out appliedCoupons);
        }
            ...
            out List<AppliedCoupon> appliedCoupons)
        {
            ...
            appliedCoupons = new List<AppliedCoupon>();
            ...
            CouponProductPriceForCaching PrepareModel()
            {
                var result = new CouponProductPriceForCaching();
                ...
                {
                    //coupons
                    List<AppliedCoupon> tmpAppliedCoupons;
                    decimal tmpCouponAmount = GetCouponAmount(product, customer, price, out tmpAppliedCoupons);
                    price = price - tmpCouponAmount;

                    if (tmpAppliedCoupons != null)
                    {
                        result.AppliedCouponIds = tmpAppliedCoupons.Select(x => x.CouponId).ToList();
                        result.AppliedCouponAmount = tmpCouponAmount;
                    }
                }
            ...
            if (includeCoupons)
            {
                foreach (var appliedCouponId in cachedPrice.AppliedCouponIds)
                {
                    var appliedCoupon = _couponService.GetCouponById(appliedCouponId);
                    if (appliedCoupon != null)
                        appliedCoupons.Add(new AppliedCoupon() { CouponId = appliedCoupon.Id, IsCumulative = appliedCoupon.IsCumulative });
                }
                if (appliedCoupons.Any())
                {
                    couponAmount = cachedPrice.AppliedCouponAmount;
                }
            }
        ...
        public virtual decimal GetCouponUnitPrice(ShoppingCartItem shoppingCartItem,
            bool includeCoupons = true)
        {
            decimal couponAmount;
            List<AppliedCoupon> appliedCoupons;
            return GetCouponUnitPrice(shoppingCartItem, includeCoupons, out couponAmount, out appliedCoupons);
        }
            ...
            out List<AppliedCoupon> appliedCoupons)
        {
            if (shoppingCartItem == null)
                throw new ArgumentNullException("shoppingCartItem");
            var product = _productService.GetProductById(shoppingCartItem.ProductId);
            return GetCouponUnitPrice(product,
                _workContext.CurrentCustomer,
                shoppingCartItem.ShoppingCartType,
                shoppingCartItem.Quantity,
                shoppingCartItem.AttributesXml,
        ...
        public virtual decimal GetCouponUnitPrice(Product product,
            Customer customer,
            ShoppingCartType shoppingCartType,
            int quantity,
            string attributesXml,
            ...
            out List<AppliedCoupon> appliedCoupons)
        {
            ...
            if (!finalPrice.HasValue)
            {
                ...
                if (attributeValues != null)
                {
                    foreach (var attributeValue in attributeValues)
                    {
                        attributesTotalPrice += GetCouponProductAttributeValuePriceAdjustment(attributeValue);
                    }
                }
                ...
                else
                {
                    ...
                    finalPrice = GetCouponFinalPrice(product,
                        customer,
                        attributesTotalPrice,
                        includeCoupons,
                        qty,
        ...
        public virtual decimal GetCouponSubTotal(ShoppingCartItem shoppingCartItem,
            bool includeCoupons = true)
        {
            decimal couponAmount;
            List<AppliedCoupon> appliedCoupons;
            return GetCouponSubTotal(shoppingCartItem, includeCoupons, out couponAmount, out appliedCoupons);
        }
           ...
           out List<AppliedCoupon> appliedCoupons)
        {
            ...
            //unit price
            var unitPrice = GetCouponUnitPrice(shoppingCartItem, includeCoupons,
                out couponAmount, out appliedCoupons);

            //coupon
            if (appliedCoupons.Any())
            {
                //we can properly use "MaximumCouponedQuantity" property only for one discount (not cumulative ones)
                Coupon oneAndOnlyCoupon = null;
                if (appliedCoupons.Count == 1)
                    oneAndOnlyCoupon = _couponService.GetCouponById(appliedCoupons.FirstOrDefault().CouponId);

                if (oneAndOnlyCoupon != null &&
                    oneAndOnlyCoupon.MaximumCouponedQuantity.HasValue &&
                    shoppingCartItem.Quantity > oneAndOnlyCoupon.MaximumCouponedQuantity.Value)
                {
                    //we cannot apply coupon for all shopping cart items
                    var couponedQuantity = oneAndOnlyCoupon.MaximumCouponedQuantity.Value;
                    var couponedSubTotal = unitPrice * couponedQuantity;
                    couponAmount = couponAmount * couponedQuantity;

                    var notCouponedQuantity = shoppingCartItem.Quantity - couponedQuantity;
                    var notCouponedUnitPrice = GetCouponUnitPrice(shoppingCartItem, false);
                    var notCouponedSubTotal = notCouponedUnitPrice * notCouponedQuantity;

                    subTotal = couponedSubTotal + notCouponedSubTotal;
                }
                else
                {
                    //coupon is applied to all items (quantity)
                    //calculate coupon amount for all items
                    couponAmount = couponAmount * shoppingCartItem.Quantity;
        ...
        public virtual decimal GetCouponProductCost(Product product, string attributesXml)
        {
            if (product == null)
                throw new ArgumentNullException("product");
        ...
        public virtual decimal GetCouponProductAttributeValuePriceAdjustment(ProductAttributeValue value)
        {
            if (value == null)
                throw new ArgumentNullException("value");
                ...
                case AttributeValueType.AssociatedToProduct:
                    {
                        ...
                        if (associatedProduct != null)
                        {
                            adjustment = GetCouponFinalPrice(associatedProduct, _workContext.CurrentCustomer, includeCoupons: true) * value.Quantity;
                        }
                    }
                    break;
                default:
