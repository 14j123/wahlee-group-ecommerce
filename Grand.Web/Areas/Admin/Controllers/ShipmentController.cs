    ...
    public partial class ShipmentController : BaseAdminController
    {
        ...
        public IActionResult SetAsDeliveredSelected(ICollection<string> selectedIds)
        {
            ...
            foreach (var shipment in shipments)
            {
                ...
                {
                    _orderProcessingService.Deliver(shipment, true);
                    Order orderd= _orderService.GetOrderById(shipment.OrderId);
                    PointWallet pointWalletd = _pointWalletService.GetPointByOrderID(shipment.OrderId);
                    pointWalletd.Activate = true;
                    if (orderd.OrderStatusId == 30)
                    {
                        pointWalletd.Order_Status = "Completed";
                    }
                    pointWalletd.ExpiredTime = DateTime.Today.AddDays(365);
                    _pointWalletService.UpdatePoint(pointWalletd);

                    //pointWalletd.CreateTime = DateTime.Now;
                    //_pointWalletService.AddPoint();
                }
                catch
                {
                    //ignore any exception
