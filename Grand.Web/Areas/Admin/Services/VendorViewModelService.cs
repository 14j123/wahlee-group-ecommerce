...
namespace Grand.Web.Areas.Admin.Services
{
    public partial class VendorViewModelService : IVendorViewModelService
    {
        ...
        public VendorViewModelService(IDiscountService discountService, ICouponService couponService, IVendorService vendorService, ICustomerService customerService, ILocalizationService localizationService,
        ...
        public virtual void PrepareCouponModel(VendorModel model, Vendor vendor, bool excludeProperties)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            model.AvailableCoupons = _couponService
                .GetAllCoupons(CouponType.AssignedToVendors, showHidden: true)
                .Select(d => d.ToModel())
                .ToList();

            if (!excludeProperties && vendor != null)
            {
                model.SelectedCouponIds = vendor.AppliedCoupons.ToArray();
            }
        }
        ...
        public Vendor InsertVendorModel(VendorModel model)
        {
            ...
            //coupons
            var allCoupons = _couponService.GetAllCoupons(CouponType.AssignedToVendors, showHidden: true);
            foreach (var coupon in allCoupons)
            {
                if (model.SelectedCouponIds != null && model.SelectedCouponIds.Contains(coupon.Id))
                    vendor.AppliedCoupons.Add(coupon.Id);
            }
