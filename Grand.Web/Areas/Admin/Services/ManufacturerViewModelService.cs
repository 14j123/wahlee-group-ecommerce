...
namespace Grand.Web.Areas.Admin.Services
{
    public partial class ManufacturerViewModelService : IManufacturerViewModelService
    {
        ...
        public virtual void PrepareCouponModel(ManufacturerModel model, Manufacturer manufacturer, bool excludeProperties)
        {
            if (model == null)
                throw new ArgumentNullException("model");

            model.AvailableCoupons = _couponService
                .GetAllCoupons(CouponType.AssignedToManufacturers, showHidden: true)
                .Select(d => d.ToModel())
                .ToList();

            if (!excludeProperties && manufacturer != null)
            {
                model.SelectedCouponIds = manufacturer.AppliedCoupons.ToArray();
            }
        }
        ...
        public virtual Manufacturer InsertManufacturerModel(ManufacturerModel model)
        {
            ...
            //coupons
            var allCoupons = _couponService.GetAllCoupons(CouponType.AssignedToManufacturers, showHidden: true);
            foreach (var coupon in allCoupons)
            {
                if (model.SelectedCouponIds != null && model.SelectedCouponIds.Contains(coupon.Id))
                    manufacturer.AppliedCoupons.Add(coupon.Id);
            }
