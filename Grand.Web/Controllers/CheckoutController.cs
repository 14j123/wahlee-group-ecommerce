...
namespace Grand.Web.Controllers
{
    public partial class CheckoutController : BasePublicController
    {
        ...
        private readonly RewardPointsSettings _rewardPointsSettings; //default grandnode
        private readonly PaymentSettings _paymentSettings;
        private readonly ShippingSettings _shippingSettings;

        private readonly IPointSettingService _pointsSettingService;
            ...
            IPointWalletService pointWalletService)
        {
            ...
            this._pointsSettingService = pointSettingService;
        ...
        public virtual IActionResult Completed(string orderId)
        {
            ...
            if (order == null)
            {
                ...
                customerId: _workContext.CurrentCustomer.Id, pageSize: 1)
                    .FirstOrDefault();
                List<PointSetting> LPS = _pointsSettingService.GetAllValidRule();
                PointSetting PS = new PointSetting();
                foreach(var getpointSetting in LPS)
                {
                    if(getpointSetting.StartDate>= DateTime.Today && getpointSetting.EndDate <= DateTime.Today)
                    {
                        if (PS == null)
                        {
                            
                            PS.Id = getpointSetting.Id;
                            PS.RuleName = getpointSetting.RuleName;
                            PS.RuleDescription = getpointSetting.RuleDescription;
                            PS.Times = getpointSetting.Times;

                        }
                        if(PS != null && PS.Times < getpointSetting.Times)
                        {
                    ...
                    if(getpointSetting.StartDate == null && getpointSetting.EndDate == null && PS == null) 
                    {
                        PS.Id = getpointSetting.Id;
                        PS.RuleName = getpointSetting.RuleName;
                        PS.RuleDescription = getpointSetting.RuleDescription;
                    ...
                    if (getpointSetting.StartDate == null && getpointSetting.EndDate == null && PS != null && PS.Times < getpointSetting.Times)
                    {
                        PS.Id = getpointSetting.Id;
                        PS.RuleName = getpointSetting.RuleName;
                        PS.RuleDescription = getpointSetting.RuleDescription;
                ...
                Wallet.PointRule_ID = PS.Id;
                Wallet.Description = PS.RuleName;
                Wallet.CreateTime = DateTime.UtcNow;
                Wallet.Customer_ID = order.CustomerId;
                Wallet.ExpiredTime = DateTime.Today.AddDays(365);
                Wallet.Activate = false;
                Wallet.Order_ID = order.Id;
                Wallet.Customer_ID = order.CustomerId;
                Wallet.Customer_Email = order.CustomerEmail;
                Wallet.Customer_Full_Name = order.FirstName+ " "+order.LastName;
                Wallet.Order_ID = order.Id;
                Wallet.Order_Number = order.OrderNumber;
                if(order.OrderStatusId == 20) { 
                    Wallet.Order_Status = "Processing";
                }
                double pointearn = Decimal.ToInt32(order.OrderSubtotalInclTax);
                double pointearn2 = pointearn * PS.Times;
                Wallet.LoyaltyPointEarn = Convert.ToInt32(pointearn2);
                _pointWalletService.AddPoint(Wallet);
