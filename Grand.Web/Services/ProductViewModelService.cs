...
namespace Grand.Web.Services
{
    public partial class ProductViewModelService : IProductViewModelService
    {
        ...
        public ProductViewModelService(IPermissionService permissionService, IWorkContext workContext, IStoreContext storeContext,
            ...
            IFlashSaleService flashSaleService, MediaSettings mediaSettings, CatalogSettings catalogSettings, SeoSettings seoSettings, VendorSettings vendorSettings, CustomerSettings customerSettings,
        ...
        public virtual IList<ProductOverviewModel> PrepareProductsDisplayedOnHomePageTrending(int? productThumbPictureSize)
        {
            var products = _productService.GetAllProductsDisplayedOnHomePageTrending();
        ...
        public virtual FlashSaleModel PrepareFlashSale(FlashSale flashSale)
        {
            var model = new FlashSaleModel
            {
                Id = flashSale.Id,
                Name = flashSale.GetLocalized(x => x.Name),
                EndTime = flashSale.EndDateUtc,
                StartTime =flashSale.StartDateUtc
            };

            var products = _productService.GetProductsByFlashSale(flashSale.Id);

            if (products != null) {

                model.FlashSaleProducts = PrepareProductOverviewModels(products).ToList();
        ...
        //see more flashSale products
        public virtual List<FlashSaleModel> PrepareHomePageFlashSale()
        {
            var cacheKey = string.Format(ModelCacheEventConsumer.HOMEPAGE_FLASHSALES_MODEL_KEY, _workContext.WorkingLanguage.Id, _storeContext.CurrentStore.Id);
            var cachedModel = _cacheManager.Get(cacheKey, () =>
                _flashSaleService.GetAllFlashSale(_storeContext.CurrentStore.Id)
                .Select(x => PrepareFlashSale(x))
                .ToList());

            var model = new List<FlashSaleModel>();
            
            foreach (var mod in cachedModel)
            {
                var flashSaleModel = (FlashSaleModel)mod.Clone();
              
                model.Add(flashSaleModel);
            }
        ...
        public virtual List<FlashSaleModel> PrepareHomePageCurrentFlashSale()
        {
         
            var currentFlashSale = _flashSaleService.GetAllCurrentFlashSale(_storeContext.CurrentStore.Id).Select(x => PrepareFlashSale(x)).ToList();

            var model = new List<FlashSaleModel>();

            foreach (var mod in currentFlashSale)
            {
                var flashSaleModel = (FlashSaleModel)mod.Clone();
